...
class Tip(models.Model):
    ...
    bookmaker = models.CharField(_('tip bookmaker (column)'), max_length=32, blank=True)
    # Translators: Coeff. (Koeficijent)
    odds = models.CharField(_('tip odds (column)'), max_length=6, blank=True)
    min_odds = models.CharField(_('minimum tip odds (column)'), max_length=6, blank=True)
    # Translators: Result (Rezultat)
    result = models.CharField(_('tip result (column)'), max_length=15, blank=True)
    # Translators: Earnings Due (Zarada)
    earnings = models.CharField(_('earnings (column)'), max_length=8, blank=True)
    # Translators: Spread (Is. Margina)
    spread = models.CharField(_('spread (column)'), max_length=8, blank=True)
    # Translators: Stake (Ulog)
    stake = models.CharField(_('stake (column)'), max_length=8, blank=True)
    # Translators: Success (Uspje\u0161nost)
    success = models.CharField(_('tip success (column)'), max_length=12, null=True, blank=True)
    tipster = models.CharField(_('tipster (column)'), max_length=12, db_index=True, blank=True)
    ...
    # large text fields
    text = models.TextField('tip text (column)', null=True, blank=True)
