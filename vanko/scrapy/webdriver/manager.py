...
class WebdriverManager(object):
    ...
    _scrapy_request_to_dict = staticmethod(reqser.request_to_dict)
    _scrapy_request_from_dict = staticmethod(reqser.request_from_dict)
    _scrapy_reqser_patched = False
    ...
    def patch_request_serialization(cls):
        assert not cls._scrapy_reqser_patched, \
            'Scrapy request serializers must be patched only once!'
        cls._scrapy_reqser_patched = True
        # cls.logger.debug('Patching scrapy request serializers')
        # print 'Patching scrapy request serializers'
        reqser.request_to_dict = cls.request_to_dict
        reqser.request_from_dict = cls.request_from_dict

    @classmethod
    def request_to_dict(cls, request, spider=None):
        dic = cls._scrapy_request_to_dict(request, spider)
        ...
        if isinstance(request, WebdriverRequest):
            ...
            dic['wd_manager'] = int(manager is not None)
            dic['wd_lock'] = int(request.lock)
        return dic

    @classmethod
    def request_from_dict(cls, dic, spider=None):
        request = cls._scrapy_request_from_dict(dic, spider)
        if not dic.get('wd_webdriver', 0):
            return request
        kwargs = {}
        for kw in ('url', 'method', 'headers', 'body', 'cookies', 'encoding',
                   'meta', 'priority', 'dont_filter', 'callback', 'errback'):
            kwargs[kw] = getattr(request, kw)
        if dic.get('wd_manager', 0):
            assert cls.global_manager is not None, \
                'WebdriverManager is not active, cannot deserialize request'
            kwargs['manager'] = cls.global_manager
        else:
            kwargs['manager'] = None
        kwargs['lock'] = bool(dic.get('wd_lock', 1))
