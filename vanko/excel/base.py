...
class ExcelProducerBase(object):
    ...
    def data_keys(self, keys, sort_by, filter_by, offset, limit, demo_limit):
        ...
        if keys is None:
            raw_keys = self.get_arg(keys, 'EXCEL_KEYS', list)
            if not raw_keys:
                raw_keys = self.redis_func('hkeys', self.table)
        else:
            raw_keys = keys
    ...
    def data_item(self, key):
        if self.get_item:
            ...
            data = self.redis_func('hget', self.table, key)
            item = self.decoder.decode(data)
        return item

    def redis_func(self, func, *args):
        assert 'Redis' in type(self.db).__name__, \
            '{} must be a Redis instance'.format(self.db)
        return getattr(self.db, func)(*args)
    ...
    def ensure_writable(self, filepath=None, can_confirm=None):
        ...
        for attempt in xrange(num_attempts):
            try:
                ...
                if (err.errno == errno.ETXTBSY and
                        can_confirm and attempt < num_attempts - 1):
                    raw_input(
                        'Please close %s and press enter... ' %
                        filepath)
                else:
                    raise
